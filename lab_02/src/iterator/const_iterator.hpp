...
class constIterator : public std::iterator<std::input_iterator_tag, T>{
    public:
        constIterator(const Matrix<T>& matrix, const size_t index=0) : __data(matrix.data), __index(index), __rows(matrix.Row()), __clms(matrix.Column()) {}
        ...
        const constIterator<T>& operator *() const {
            ...
            if (__data.expired()) {
                throw NullPtrException(__FILE__, typeid(*this).name(), __LINE__, curr_time, "Data pointer is null.");
            }

            if (__index >= __rows * __clms) {
                throw indexException(__FILE__, typeid(*this).name(), __LINE__, curr_time, "Const iterator out of bounds.");
            }
